...
The goal of SweetPea is to allow scientists to easily describe experimental designs and to synthesize experimental sequences which satisfy those designs in a statistically rigerous way. To do this, SweetPea is split into a language, which aims to provide semantics which mirror the terms researchers use when describing their experiments, and a runtime which synthesizes the sequences. Because the runtime encode the experimental design as a SAT formula, the language must both be convineint for domain users and amenable to encoding in SAT.

In this chapter we'll take a close look at the features of the SweetPea language. We will look at motivating examples, how those examples are represented in SweetPea. In the next chapter we'll look at how these language constructs are encoded in SAT.
...
- full crossing = counting constraints
