...
Another relationship is a balancing constraint. A pair of levels is balanced if each instance shows up the same number of times; a full crossing of all levels is a fully balanced experiment. Currently the only mechanism for balancing in SweetPea is including factors in a full-crossing.
...
The remaining subsections will discuss this schema, broken up into sections.
...
In the resulting experimental sequence, exactly one choice of level is selected per trial.

Nested levels are handled in exactly the same way- nesting is just a way to reference a grouping in the high-level representation. In the resulting experimental sequence, nesting is flattened, and exactly one choice of level is selected.
...
con_level  = DerivedLevel("rep",
               WithinTrial(congruent, [ink_color, ink_color]))
inc_level  = DerivedLevel("swi",
               WithinTrial(congruent, [ink_color, ink_color], False))
